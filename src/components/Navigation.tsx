      ...
      { 
        ...
        submenu: [
          ...
          },
          {
            title: 'Gaming',
            href: '/products/gaming',
            description: 'Popular gaming accessories'
          }
        ]
      },
    ],
...
export function Navigation() {
  ...
  const isSticky = deviceType === 'desktop' || deviceType === 'tablet';

  const isItemAvailable = (item: NavItem) => {
    if (!item.availableDevices) return true;
    return item.availableDevices.includes(deviceType);
  };

  const getNavigationBehavior = () => {
    if (capabilities.hasMouse && capabilities.hasHover) {
      return 'hover';
    }
    return 'click';
  };
      ...
      {navItems.map((item) => (
        isItemAvailable(item) && (
          <div key={item.title} className="relative group">
            ...
            {item.submenu && (
              <div 
                className={`absolute top-full left-0 w-[275px] p-2 mt-2 bg-white dark:bg-gray-900 rounded-lg shadow-lg border border-gray-200 dark:border-gray-800 ${
                  getNavigationBehavior() === 'hover' 
                  ...
                  {item.submenu.map((subItem) => (
                    isItemAvailable(subItem) && (
                      <div key={subItem.title} className="relative group/submenu">
                        ...
                        {subItem.submenu && (
                          <div 
                            className={`absolute left-full top-0 w-[275px] p-2 ml-2 bg-white dark:bg-gray-900 rounded-lg shadow-lg border border-gray-200 dark:border-gray-800 ${
                              getNavigationBehavior() === 'hover'
                                ? 'opacity-0 invisible group-hover/submenu:opacity-100 group-hover/submenu:visible transition-all duration-200'
  ...
  return (
    <header className={`w-full ${isSticky ? 'fixed top-0 z-50' : 'relative z-50'} bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm shadow-sm`}>
      <div className={`max-w-7xl mx-auto px-2 sm:py-6 ${deviceType === 'smartwatch' ? 'py-2' : 'px-4 py-2'}`}>
        <div className="flex items-center justify-between">
